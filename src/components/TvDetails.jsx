...
import { asyncAddtv, removeTv } from '../store/actions/tvAction'
  ...
  return info ? (
    <div className='bg-black pb-4'>
      <div style={{
        ...
        <div className="flex flex-col md:flex-row items-center justify-around mt-3 px-0 lg:px-10">
          <img src={info.detail.poster_path || info.detail.backdrop_path || info.detail.profile_path ? `https://image.tmdb.org/t/p/original/${info.detail.poster_path || info.detail.backdrop_path || info.detail.profile_path}` : noImage} alt="" className='w-[280px] lg:w-[340px] h-[420px] lg:h-[480px] shadow-[8px_20px_32px_2px_rgba(0,0,0, 0.5)] object-cover' />
          <div className="text-zinc-200 w-[96%] sm:w-[92%] md:w-[60%] mt-3 md:mt-0">
            ...
            <div className="flex gap-10 mt-2 lg:mt-3">
              <p><FontAwesomeIcon icon={faCalendarDays} className='mr-2 text-yellow-400 text-xl' />{info.detail.first_air_date
                || 'N/A'}</p>
              <p><FontAwesomeIcon icon={faCompactDisc} className='mr-2 text-yellow-400 text-xl' />tv</p>
            </div>
            <div className="flex items-center">
              <h1 className='mt-1 lg:mt-3 text-1xl font-semibold mr-3'>Genres:</h1>
              {
                info.detail.genres.map((g, i) => <p key={i} className='mr-2 mt-1 lg:mt-3 text-sm lg:text-md' >{g.name},</p>)
              }
            </div>
            <div className="flex items-center mt-2 lg:mt-3"><h1 className='text-1xl font-semibold mr-3'>Duration:</h1>{info.detail.runtime} min.</div>
            <h1 className='mt-2 lg:mt-3 text-1xl font-semibold'>tv Translated:</h1>
            <p className='mt-1 text-sm lg:text-md'>{info.translations.map((t) => t.english_name).join(', ')}</p>
            <div className="flex items-center justify-start">
              <Link to={`/tv/details/${info.detail.id}/trailer`} className="flex justify-center items-center relative">
                <FontAwesomeIcon icon={faCirclePlay} style={{ color: "#74C0FC", }} className='text-4xl lg:text-5xl mt-2 lg:mt-4 z-10 shadow-xl' />
                <span className='bg-gradient-to-r from-cyan-500 to-blue-500 py-[1px] lg:py-[3px] pl-2 lg:pl-3 rounded-r-full absolute w-[100px] lg:w-[110px] text-sm lg:text-base left-8 lg:left-10 top-[15px] lg:top-[25px] text-black'>Watch Trailer</span>
              </Link>
            </div>
          </div>
        </div>
        <div className="flex flex-col px-3 md:px-20 pt-2 md:pt-5 text-zinc-200">
      ...
      <div className="px-10 bg-black">
        ...
        <div className="flex gap-3 justify-around h-full p-4 overflow-x-auto overflow-hidden">
          {
            info.detail.seasons.length > 0 ? info.detail.seasons.map((s, i) => <div  key={i}>
              <img src={`https://image.tmdb.org/t/p/original/${s.poster_path}`} alt="" className='bg-zinc-900 min-w-[200px] overflow-hidden h-[280px]' />
      ...
      <hr className='border-none bg-zinc-400 h-[2px] w-[95%] mx-auto my-5' />
      <div className="px-3 md:px-10 mt-2">
        <h1 className='mt-1 md:mt-3 text-xl md:text-3xl font-semibold text-white'>Recommendations & Similar Stuff:</h1>
