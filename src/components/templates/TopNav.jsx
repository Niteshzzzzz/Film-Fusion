    ...
    return (
        <div className='z-50 flex w-[100%] justify-center items-center mt-10 sm:mt-5 relative'>
            ...
            <div className="w-[59%] bg-zinc-100 max-h-[50vh] absolute top-[104%] overflow-auto rounded-lg">
                {
                    searches.map((s, i) => 
                        <Link to={`/${s.media_type}/details/${s.id}`} key={i} className='py-2 sm:py-4 px-5 sm:px-10 flex items-center justify-start bg-zinc-200 border-b-2 border-zinc-400 text-zinc-700 font-semibold text-sm sm:text-1xl hover:bg-zinc-300 hover:text-black duration-300'>
                            <img src={s.backdrop_path || s.profile_path ? `https://image.tmdb.org/t/p/original/${ s.backdrop_path || s.profile_path}`: noImage} alt="" className='w-[70px] sm:w-[9vw] h-[50px] sm:h-[9vh] object-cover mr-3 shadow-lg' />
