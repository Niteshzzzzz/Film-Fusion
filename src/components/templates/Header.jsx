...
function Header({ wallpaper }) {
    return (
        <div className='w-full h-[50vh] mt-5 text-white px-5 py-10 flex items-start justify-end flex-col' style={{
            ...
            <p className='mt-4 w-[80%]'>{wallpaper.overview.slice(0, 200)}... <Link to={`/${wallpaper.media_type}/details/${wallpaper.id}`} className='text-blue-400'>more.</Link></p>
            ...
            <Link to={`/${wallpaper.media_type}/details/${wallpaper.id}`} className="flex justify-center items-center relative">
